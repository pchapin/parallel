/*!
 * \file   linear_equations-bidirectional.c
 ...
 * Unlike the linear_equations-barriers version, this version switches the scan direction after
 * each pass of the outer loop. This is intended to improve cache utilization by reusing values
 * recently loaded into the cache.
 */
...
void *bidirectional_row_processor( void *arg )
{
    enum direction_t { DOWN, UP };
    ...
    while( 1 ) {
        ...
        if( direction == DOWN ) {
            ...
            for( int j = stop_row - 1; j >= start_row; --j ) {
                m = MATRIX_GET( a, size, j, base_row ) / MATRIX_GET( a, size, base_row, base_row );
                for( int k = 0; k < size; ++k )
                    MATRIX_PUT( a, size, j, k, MATRIX_GET( a, size, j, k ) - m * MATRIX_GET( a, size, base_row, k ) );
                b[j] -= m * b[base_row];
...
static int elimination( int size, floating_type *a, floating_type *b )
{
    ...
    for( k = 0; k < processor_count; ++k )
        pthread_create( &thread_IDs[k], NULL, bidirectional_row_processor, &work_units[k] );
