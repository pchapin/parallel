 ...
 */
void solver( SudokuBoard &puzzle, const int current_row, const int current_col, unsigned available_threads )
{
    int next_row;
    int next_col;
    std::vector<boost::thread *> worker_threads;
    std::vector<SudokuBoard *>   worker_boards;
        ...
        if( puzzle.incremental_valid_board( current_row, current_col ) ) {
            ...
            if( puzzle.filled == puzzle.n * puzzle.n ) {
                ...
                if( available_threads == 1 )
                    solver( puzzle, next_row, next_col, available_threads );
                else {
                    worker_boards.push_back( new SudokuBoard( puzzle ) );
                    worker_threads.push_back( new boost::thread( solver, *worker_boards.back( ), next_row, next_col, 1 ) );
                    --available_threads;
                }
            }
        }
...
execute_solve:
        solver( *this, next_row, next_col, boost::thread::hardware_concurrency( ) );
    }
